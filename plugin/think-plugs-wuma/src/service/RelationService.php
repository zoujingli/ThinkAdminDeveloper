...
class RelationService extends Service
{
    ...
    public static function resetAssignLock(string $batch, bool $relation = false): bool
    {
        ...
        try {
            if ($relation) { /* 关联赋码 */
                foreach ($batchs as $pbatch => $codes) {
                    foreach ($codes as $code) {
                        $items[] = [
                            'real' => '',
                            'lock' => '',
    ...
    public static function changeAssignLock(array $mins, ?int $lock = null): array
    {
        ...
        foreach ($model->whereOr($map)->cursor() as $item) {
            foreach ($mins as $k => $min) {
    ...
    {
        if (empty($codes)) {
            throw new Exception('无效的标签数据！');
        }
        $produce = PluginWumaSourceProduce::mk()->where(['batch' => $pbatch])->findOrEmpty();
        if ($produce->isEmpty()) {
            throw new Exception('无效的生产批次数据！');
        }
    ...
    private static function withMergeInput(array $ranges, array $inputs, string $pbatch): array
    {
        ...
        foreach ($ranges as $range) {
            ...
            while (count($inputs) > 0) {
                if ($range[0] > $inputs[0] || $inputs[0] > $range[1]) {
                    break;
                }
