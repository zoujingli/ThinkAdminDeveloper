...
class Assign extends Controller
{
    ...
    public function index()
    {
        PluginWumaSourceAssign::mQuery()->layTable(function () {
            ...
            // 物码数值批次筛选
            if (!empty($this->get['encode'])) if (is_numeric($this->get['encode'])) {
    ...
    protected function _form_filter(array &$data)
    {
        ...
        if ($this->request->isGet()) {
            ...
            if (empty($this->coders)) $this->error('物码批次不能为空！');
            if (empty($this->produces)) $this->error('生产批次不能为空！');
            // 关联赋码区间
            ...
            $items = [];
            foreach (json_decode($data['items'], true) as $item) $items[] = [
                'real'        => $item['real'] ?? 0,
                'lock'        => $item['lock'] ?? 0,
                'batch'       => $data['batch'],
                'pbatch'      => $item['batch'],
    ...
    public function mode()
    {
        ...
        if ($assign->isEmpty()) $this->error('无效的批次码！');
