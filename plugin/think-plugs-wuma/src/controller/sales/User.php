...
class User extends Controller
{
    ...
    public function index()
    {
        PluginWumaSalesUser::mQuery()->layTable(function () {
            ...
            if ($db->getOptions('where')) $query->whereRaw("auid in {$db->field('id')->where($map)->buildSql()}");
            // 当前代理查询
    ...
    protected function _form_filter(array &$data)
    {
        ...
        if ($this->request->isGet()) {
            $this->levels = PluginWumaSalesUserLevel::lists();
            if (empty($this->levels)) $this->error('请先添加代理等级！');
            // 有效时间范围处理
            if (empty($data['super_phone'])) $data['super_phone'] = $this->get['from'] ?? '';
            if (empty($data['date'])) [$data['date_start'], $data['date_after']] = [date('Y-m-d'), date('Y-m-d', strtotime('+1year'))];
        } elseif ($this->request->isPost()) {

            // 检查手机号是否出现重复
            $model = PluginWumaSalesUser::mk()->where(['phone' => $data['phone'], 'deleted' => 0]);
            if (isset($data['id'])) $model->whereRaw("id<>{$data['id']}");
            if ($model->count() > 0) $this->error("手机号已经存在，请使用其它手机！");
            ...
            // 有效时间范围处理
            if (empty($data['date'])) $this->error('授权时间不能为空！');
            ...
            if (!empty($data['super_phone'])) {
                ...
                if (empty($user)) $this->error('邀请人手机号异常！');
                $data['auid'] = $user['id'];
