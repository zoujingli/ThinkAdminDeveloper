...
class MediaService extends Service
{
    ...
    {
        // 文章主体数据
        $data = WechatNews::mk()->where(['id' => $id])->where($map)->findOrEmpty()->toArray();
        if (empty($data)) {
            return [];
        }
        ...
        // 文章内容列表
        $items = WechatNewsArticle::mk()->whereIn('id', $aids)->withoutField('create_by,create_time')->select()->toArray();
        foreach ($aids as $aid) {
