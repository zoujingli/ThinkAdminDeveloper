...
class QueueService extends Service implements QueueManagerInterface, QueueHandlerInterface
{
    ...
    /**
     * Enter queue runtime context for the current process.
     */
    ...
    /**
     * Leave queue runtime context for the current process.
     */
    ...
    /**
     * Check whether the current process is executing a queue task.
     */
    ...
    /**
     * Get current runtime queue code.
     */
    ...
    public function reset(int $wait = 0): QueueService
    {
        ...
        $this->record->save(['exec_pid' => 0, 'exec_time' => time() + $wait, 'status' => static::STATE_WAIT]);
        return $this;
    }
    ...
    {
        return static::register('定时清理系统任务数据', 'xadmin:queue clean', 0, [], 0, $loops);
    }
    ...
    {
        try {
            $map = [['title', '=', $title], ['status', 'in', [static::STATE_WAIT, static::STATE_LOCK]]];
    ...
    public function registerTask(string $title, string $command, int $later = 0, array $data = [], int $rscript = 0, int $loops = 0): QueueManagerInterface
    {
        return static::register($title, $command, $later, $data, $rscript, $loops);
    }
    ...
    public function message(int $total, int $count, string $message = '', int $backline = 0): void
    {
        ...
        if (static::inContext($this->code)) {
            $this->progress(static::STATE_LOCK, "[{$prefix}/{$total}] {$message}", sprintf('%.2f', $count / max($total, 1) * 100), $backline);
        } else {
    ...
    private function _lazyWrite(bool $force = false): array
    {
        ...
        // 消息延时写数据库
        if ($force || empty($this->msgs['sctime']) || in_array($this->msgs['status'], [static::STATE_DONE, static::STATE_ERROR], true) || microtime(true) - $this->msgs['sctime'] > 1) {
