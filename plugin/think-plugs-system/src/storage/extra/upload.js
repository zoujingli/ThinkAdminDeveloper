(function (SparkMD5, Notify) {
    let allowMime = JSON.parse('{$exts|raw}');
    ...
    function UploadAdapter(elem, done) {
        return new (function (elem, done) {
            ...
            this.adapter = new Adapter(this.option, layui.upload.render({
                url: '{:apiuri("system/upload/file",[],false,true)}', auto: false, elem: elem, accept: 'file', multiple: this.option.mult, exts: this.option.exts.join('|'), acceptMime: this.option.mimes.join(','), choose: function (obj) {
    ...
    Adapter.prototype.upload = function (files, done) {
        ...
        layui.each(files, function (index, file) {
            ...
            if (!isGif && /^image\//.test(file.type) && (file.maxWidth + file.maxHeight + file.cutWidth + file.cutHeight > 0 || file.quality !== 1)) {
                $.module.use(['compressor'], function (Compressor) {
    ...
    Adapter.prototype.request = function (file, done) {
        ...
        jQuery.ajax("{:apiuri('system/upload/state',[],false,true)}", {
            data: data, method: 'post', success: function (ret) {
                ...
                if (parseInt(ret.code) === 404) {
                    ...
                    uploader.form.append('file', file, file.name), jQuery.ajax({
                        headers: uploader.head, url: uploader.url, data: uploader.form, type: uploader.type || 'post', xhr: function (xhr) {
    ...
    Adapter.prototype.done = function (ret, idx, file, done, message) {
        ...
        jQuery.post("{:apiuri('system/upload/done',[],false,true)}", {id: file.id, hash: file.xmd5});
        /*! 单个文件上传成功结果处理 */
    ...
    window.taUploadModule = UploadAdapter;
...
})(window.SparkMD5, window.Notify || window.GrowlNotification);
