...
class AppService extends Service
{
    ...
    public static function all(bool $force = false): array
    {
        ...
        $apps = array_merge(self::local($force), self::plugins());
        ksort($apps);
    ...
    public static function local(bool $force = false): array
    {
        ...
        $apps = [];
        $basePath = Library::$sapp->getBasePath();
        foreach (scandir($basePath) ?: [] as $code) {
            if ($code[0] === '.') {
                continue;
            }
            $path = $basePath . $code . DIRECTORY_SEPARATOR;
            if (!is_dir($path) || in_array($code, self::IGNORE_LOCAL_APPS, true)) {
                continue;
            }
            if (!self::isLocalAppPath($path)) {
                continue;
            }
    ...
    {
        $apps = [];
        foreach ((array)Plugin::get() as $code => $app) {
            $apps[$code] = self::normalize($code, ['type' => 'plugin'] + $app);
        }
        ksort($apps);
        return $apps;
    }
