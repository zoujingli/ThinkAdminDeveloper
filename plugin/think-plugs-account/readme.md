...
**ThinkPlugsAccount** 是 **ThinkAdmin** 的多端账号插件，用于管理终端账号、手机号绑定、正式会员账号和接口令牌。本插件属于[会员尊享插件](https://thinkadmin.top/vip-introduce)，未经授权不得用于商业用途。

当前插件提供微信服务号、微信小程序、手机短信验证等账号接入方式，并可通过动态通道扩展其他终端账号类型。

账号数据以终端账号 `usid`、绑定手机号 `bind`、正式用户 `unid` 三层关系组织，适用于公众号、小程序、APP、H5、PC 和自定义终端等多端登录场景。
...
- **用户账号编号 - unid**：正式用户账号编号，对应 `plugin_account_user` 表的 `id` 字段。通过绑定和解绑操作，可将多个终端账号关联到同一正式用户。
- **终端账号编号 - usid**：用户在某个终端通道下的登录身份编号，对应 `plugin_account_bind` 表的 `id` 字段。一个终端账号只能绑定一个正式用户账号。
