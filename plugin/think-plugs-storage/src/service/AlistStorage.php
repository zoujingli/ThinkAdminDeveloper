...
class AlistStorage implements StorageInterface
{
    ...
    public function set(string $name, string $file, bool $safe = false, ?string $attname = null): array
    {
        ...
        $result = HttpClient::submit("{$this->domain}/api/fs/form", [], $file, $header, 'PUT', false);
    ...
    private function httGet(string $uri): array
    {
        ...
        $result = HttpClient::get($this->domain . $uri, [], ['headers' => $header]);
        if (is_array($data = json_decode($result, true))) {
            if ($data['code'] === 200 && $data['message'] === 'success') {
                return $data;
            }
    ...
    private function httpPost(string $uri, array $body = [], bool $auth = true): array
    {
        ...
        $header[] = 'Content-Type: application/json;charset=UTF-8';
        $result = HttpClient::post($this->domain . $uri, $body, ['headers' => $header]);
        if (is_array($data = json_decode($result, true))) {
            if ($data['code'] === 200 && $data['message'] === 'success') {
                return $data;
            }
